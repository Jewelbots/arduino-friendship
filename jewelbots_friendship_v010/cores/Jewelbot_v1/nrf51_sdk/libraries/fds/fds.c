...
#define OLD_PAGE_END_ADDR  ((uint32_t*)(243456))
#define OLD_PAGE_START_ADDR (OLD_PAGE_END_ADDR - (FDS_VIRTUAL_PAGES * FDS_PAGE_SIZE))
...
static bool record_find_next(uint16_t page, uint32_t const ** p_record)
{
           ...
           *p_next_rec != FDS_ERASED_WORD))
    {
        ...
        {
            //NRF_LOG("RECORD NOT VALID\r\n");
            // The record is not valid, jump to the next.
            p_next_rec += (FDS_HEADER_SIZE + (p_header->tl.length_words));
        }
    }
    //NRF_LOG("NO MORE VALID RECORDS\r\n");
...
static fds_init_opts_t pages_init()
{
    ...
    for (uint16_t i = 0; i < FDS_VIRTUAL_PAGES; i++)
    {
        uint32_t        const * const p_page_addr = fs_config.p_start_addr + (i * FDS_PAGE_SIZE);
				uint32_t        const * const old_p_page_addr = OLD_PAGE_START_ADDR + (i * FDS_PAGE_SIZE);
        fds_page_type_t const         page_type   = page_identify(p_page_addr);
				fds_page_type_t const         old_page_type   = page_identify(old_p_page_addr);

        switch (page_type)
        {
            case FDS_PAGE_UNDEFINED:
								//NRF_LOG_DEBUG("page undefined\r\n");
                if ((old_page_type != FDS_PAGE_UNDEFINED) || (page_is_erased(p_page_addr)))
                {
										//NRF_LOG_DEBUG("page is erased\r\n");
                    if (m_swap_page.p_addr != NULL)
                    {
												//NRF_LOG_DEBUG("swap page NOT null\r\n");
                    ...
                    {
												//NRF_LOG_DEBUG("swap page is null\r\n");
                ...
                {
										//NRF_LOG_DEBUG("page is NOT erased\r\n");
