...
fs_ret_t fs_init(void)
{
    ...
    uint32_t const * p_current_end = FS_PAGE_END_ADDR;
		//uint32_t const * test_end	= FS_PAGE_END_ADDR2;
    uint32_t         index_max     = 0x00;
    uint32_t         index_last    = 0xFFFFFFFF;

		//NRF_LOG_PRINTF_DEBUG("p_current_end = %u\r\n", p_current_end);
		//NRF_LOG_PRINTF_DEBUG("test_end = %u\r\n", test_end);
                  ...
                  uint16_t    const         num_pages)
{
    ...
    {
				NRF_LOG_PRINTF_DEBUG("p_page_addr = %u\r\n", (uint32_t)p_page_addr);
        return FS_ERR_UNALIGNED_ADDR;
    }
