...
unsigned char haptics_test_run2(void) {
  ...
  uint32_t err_code =
      app_timer_start(haptics_timer_id, BUZZALERT_TIMEOUT, NULL);
  APP_ERROR_CHECK(err_code);
  return 0;
}
...
unsigned char haptics_test_run3(void) {
  ...
  uint32_t err_code =
      app_timer_start(haptics_timer_id, DOUBLESTRONGCLICK_TIMEOUT, NULL);
  APP_ERROR_CHECK(err_code);
  return 0;
}
...
unsigned char haptics_test_run4(void) {
  ...
  uint32_t err_code =
      app_timer_start(haptics_timer_id, PULSINGALERT_TIMEOUT, NULL);
  APP_ERROR_CHECK(err_code);
  return 0;
}
